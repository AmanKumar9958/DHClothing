...
const LazyImage = ({ src, alt, className, skeletonClass = "w-full h-full min-h-[200px]", wrapperClassName, onClick, ...props }) => {
  ...
  return (
    <div 
        className={`relative ${!isLoaded ? 'bg-gray-100' : ''} ${wrapperClassName || ''}`}
        onClick={onClick}
    >
      {!isLoaded && (
        <div className={`absolute inset-0 flex items-center justify-center bg-gray-200 animate-pulse rounded ${skeletonClass}`}>
